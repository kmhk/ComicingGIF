...
- (BOOL) setVideo:(NSString *)videoFileName {
    ...
    NSString *path = videoFileName;//[[NSBundle mainBundle] pathForResource:self.videoFileName ofType:@"mp4"];
    if (path == nil) {
        NSLog(@"File is not found");
        return FALSE;
    }
    
    return [self setVideoData:[NSData dataWithContentsOfURL:[NSURL URLWithString:path]]];
}
